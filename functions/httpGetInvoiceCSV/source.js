httpGetDigest = function(url, authHeader) {
  ...
  try {
      return context.http.get({ url: url, headers: { 'Authorization': [ digestHeader ], 'Accept': ['text/csv'] } });
  } catch (err) {
    console.log("httpGetDigest() error: ", err)
  }
};
...
exports = function(url) {
  ...
  try {
    ...
    console.log("httpGetInvoiceCSV() error: ", err)
  }
